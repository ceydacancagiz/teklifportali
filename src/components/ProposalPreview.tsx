...
const ProposalPreview = forwardRef<HTMLDivElement, Props>(
  ({ data, getCurrencySymbol, calculateTotal }, ref) => {
    ...
    return (
      <div ref={ref}>
        {/* ===== PAGE 1: Main proposal content ===== */}
        <div data-page="1" style={PAGE_STYLE}>
          <div style={{ padding: "24px 32px", display: "flex", flexDirection: "column", height: "100%", minHeight: "1075px" }}>
            {/* Date */}
            <div style={{ textAlign: "right", fontSize: "11px", marginBottom: "16px" }}>
              {data.date}
            </div>
            ...
            {/* Intro */}
            <p style={{ fontSize: "10px", lineHeight: "1.5", marginBottom: "20px" }}>
              {data.introText}
            </p>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "16px" }}>
              <thead>
                <tr style={{ borderBottom: "2px solid #000" }}>
                  <th style={{ textAlign: "left", padding: "6px 4px", fontSize: "10px", fontWeight: "bold" }}>AÇIKLAMA</th>
                  <th style={{ textAlign: "center", padding: "6px 4px", fontSize: "10px", fontWeight: "bold", width: "80px" }}>MİKTAR</th>
                  <th style={{ textAlign: "right", padding: "6px 4px", fontSize: "10px", fontWeight: "bold", width: "100px" }}>BİRİM FİYAT</th>
                  <th style={{ textAlign: "right", padding: "6px 4px", fontSize: "10px", fontWeight: "bold", width: "100px" }}>TOPLAM</th>
                </tr>
              </thead>
              <tbody>
                {data.lineItems.map((item, index) => (
                  <tr key={item.id} style={{ borderBottom: "1px solid #ccc" }}>
                    <td style={{ padding: "5px 4px", fontSize: "10px", fontWeight: "bold" }}>{index + 1}. {item.description || "-"}</td>
                    <td style={{ padding: "5px 4px", fontSize: "10px", textAlign: "center" }}>{item.quantity}</td>
                    <td style={{ padding: "5px 4px", fontSize: "10px", textAlign: "right" }}>{formatCurrency(item.unitPrice)}</td>
                    <td style={{ padding: "5px 4px", fontSize: "10px", textAlign: "right" }}>{formatCurrency(item.quantity * item.unitPrice)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            {/* Footer Note */}
            {data.footerText && (
              <p style={{ fontSize: "8px", fontStyle: "italic", lineHeight: "1.5", marginBottom: "16px" }}>
                {data.footerText}
              </p>
            )}
        ...
        {/* ===== PAGE 2+: Kit Lists ===== */}
        {hasKitLists && (
          <div data-page="kit-lists" style={{ ...PAGE_STYLE, minHeight: "auto" }}>
            <div style={{ padding: "24px 32px" }}>
              {data.lineItems.map(
                (item, itemIndex) =>
                  item.kitList.length > 0 && (
                    <div key={item.id} data-kit-section="true" style={{ marginBottom: "16px" }}>
                      <div
                        style={{
        ...
        {/* Additional Notes when no kit lists */}
        {!hasKitLists && data.additionalNotes && (
          <div data-page="notes" style={{ ...PAGE_STYLE, minHeight: "auto" }}>
            <div style={{ padding: "24px 32px" }}>
              <div style={{ marginBottom: "12px" }}>
                <h3 style={{ fontWeight: "bold", fontSize: "11px", marginBottom: "3px" }}>
                  İLAVE NOTLAR
